                        ...
                        <div className="lg:col-span-8">
                            ...
                            <div className="grid grid-cols-2 gap-4 mb-20">
                                {property.images.slice(1, 5).map((img, idx) => (
                                    <div key={idx} className="relative h-64 md:h-80 rounded-[2rem] overflow-hidden shadow-lg group">
                                        <Image
                                            src={img}
                            ...
                            <div className="space-y-12 pb-12">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    {property.nearby.map((place, idx) => (
                                        <div key={idx} className="bg-card p-6 rounded-3xl border border-border/50 flex items-center gap-5 hover:border-primary/50 transition-colors">
                                            <div className="h-12 w-12 rounded-2xl bg-muted flex items-center justify-center text-muted-foreground">
                                                <MapPin size={24} />
                                            </div>
                                            <div>
                                                <h4 className="font-black text-foreground uppercase text-sm mb-1">{place.name}</h4>
                                                <p className="text-xs text-muted-foreground font-bold italic uppercase tracking-widest">{place.type} • {place.distance}</p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                                    ...
                                    <div className="relative z-10">
                                        ...
                                        <div className="space-y-4 mb-8">
                                            <div className="flex justify-between items-center py-3 border-b border-border">
                                                <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Price per SqFt</span>
                                                <span className="text-sm font-black text-foreground">£{property.pricePerSqft}</span>
                                            </div>
                                            <div className="flex justify-between items-center py-3 border-b border-border">
                                                <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Energy Rating</span>
                                                <span className="text-sm font-black text-foreground">{property.energyRating}</span>
                                            </div>
                                            <div className="flex justify-between items-center py-3 border-b border-border">
                                                <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Council Tax</span>
                                                <span className="text-sm font-black text-foreground">{property.councilTax}</span>
                                            </div>
                                        </div>

                                        <button className="primary-btn w-full py-5 rounded-2xl font-black uppercase tracking-[0.2em] shadow-xl shadow-primary/20 hover:shadow-primary/40 transition-all duration-300 mb-4 flex items-center justify-center gap-3">
                                            Inquire Now
                                            <ArrowRight size={20} />
                                        </button>
                                        ...
                                        <Link
                                            ...
                                            className="w-full bg-[#25D366] hover:bg-[#128C7E] text-white py-5 rounded-2xl font-black uppercase tracking-[0.2em] shadow-xl shadow-green-500/20 flex items-center justify-center gap-3 transition-all duration-300"
                                        >
